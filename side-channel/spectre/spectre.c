...
*
* Modified: Cheolwoo Myung, 10/27/2022
* 
**********************************************************************/
...
uint8_t unused2[64];
uint8_t array2[256*512]; /* size: 32KB */
uint8_t temp = 0; /* Used so compiler won't optimize out victim_function() */

char *secret = "The Magic Words are Seoul National University.";
...
void readMemoryByte(int cache_hit_threshold, size_t malicious_x, uint8_t value[2], int score[2]) {
    ...
    for (tries = 999; tries > 0; tries--) {
        ...
        /* 30 loops: 5 trainig runs (x=training_x) per attack run (x=malicious_x) */
        ...
        for (i = 0; i < 256; i++) {
            ...
            The best way to do this is to use the rdtscp instruction, which measure current
...
int main(int argc, char** argv) {
    ...
    while (--len > 0) {
        printf("Reading at malicious = %p... ", (void*)(array1+malicious_x));
